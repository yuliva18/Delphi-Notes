...
procedure TForm1.AddNote(q: TFDQuery);
var n: Note;
begin
  n := Note.Create();
  n.Init(Self, q.FieldByName('id').AsInteger, q.FieldByName('body').AsString, q.FieldByName('date').AsString);
...
begin
  ...
  q.ExecSQL();
  note.noteFrame.BodyLabel.Caption := note.body.Split([#$D#$A])[0];
end;
